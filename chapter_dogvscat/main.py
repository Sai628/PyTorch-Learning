...
def train(**kwargs):
    ...
    vis.log("epoch:{epoch}, lr:{lr}, loss:{loss}, train_cm:{train_cm}, val_cm:{val_cm}"
            .format(epoch=epoch, lr=lr, loss=loss_meter.value().item()),
                    train_cm=str(confusion_matrix.value()), val_cm=str(val_cm.value()))

    # 如果损失不再下降, 则降低学习率
    if loss_meter.value().item() > previous_loss:
        lr = lr * opt.lr_decay
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr

    previous_loss = loss_meter.value().item()
...
def test(**kwargs):
    ...
    # 开始测试
    results = []
    for i, (data, path) in tqdm(enumerate(test_dataloader)):
